...
class UserController extends Controller {
    ...
    public function view_profile(Request $request) {
        if (!Auth::check()) {
            ...
            $regionadvetisement = $this->post->RegionAdvertisement($request->location);
            ...
            return view('view_profile', compact('info', 'neighbourpost','regionadvetisement'));
        }
    }
    ...
    public function addFollower(Request $request) {
        ...
        if ($subscriberCount == 0) {
            ...
            if (strpos($l, ',') !== false) {
                echo \GuzzleHttp\json_encode(array('status' => 'success', 'url' => route('home') . '/' . sanitizeStringForUrl($user_region->name) . '/' . sanitizeStringForUrl($user_community->name) . '/register?utm_source=invite-a-friend&utm_medium=web&utm_campaign=invite', 'message' => 'Subscribed successfully.!'));
            } else {
